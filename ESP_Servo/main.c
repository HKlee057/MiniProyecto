...
{
    memset(ReceivedData, 0, sizeof(ReceivedData));
    SendATCommand("AT+CWJAP=\"CLARO_4EE7A9\",\"31DB20D70D\""); //Mi Wifi: NetworkName, Password
...
{
    memset(ReceivedData, 0, sizeof(ReceivedData));
    SendATCommand("AT+CIPSTART=\"TCP\",\"192.168.1.14\",9999"); //Server IP and Port: such as 192.255.0.100, 9999
    return recvFind("OK",2000, true);
}
...
int main(void)
{
    ...
    while(true)
    {
        UARTprintf("Conectando a wi-fi\n");
        TaskStatus = CWJAPesp();
        if(TaskStatus)
        {
            UARTprintf("Conexion establecida\n");
            break;
        }
    }
...
bool ProcessRoutine()
{
    ...
    while(true)
    {
        ...
        if(status)
        {
            UARTprintf("Comunicacion establecida\n");
            break;
        }
